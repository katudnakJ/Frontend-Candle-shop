  ...
  return (
    <div className="space-y-4">
      <div className="flex   max-[470px]:flex-col justify-between items-end max-[470px]:items-start px-1">
        ...
        <span className="flex  text-xs font-bold text-red-500">* จำเป็นต้องอัปโหลดรูปอย่างน้อย 1 รูป</span>
      </div>
      ...
      <div className="grid grid-cols-12 grid-rows-4 gap-3 aspect-[4/3] w-full">
        ...
        <div className="col-span-8 row-span-4 relative">
          {images[0] ? (
            ...
            >
              ...
              <p className="text-xs font-black text-gray-400">เพิ่มรูปภาพหลัก</p>
            </div>
          )}
        </div>
        ...
        {/* รูปย่อย  */}
        {[1, 2].map((idx) => (
          <div key={idx} className="col-span-4 row-span-2 relative">
            {images[idx] ? (
              <div className="w-full h-full rounded-2xl border-2 border-black overflow-hidden bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative">
                ...
                <button
                  ...
                  className="absolute top-1 right-1 bg-red-500 text-white p-1 rounded-lg border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none transition-all"
                >
                  <X size={14} />
                </button>
              </div>
